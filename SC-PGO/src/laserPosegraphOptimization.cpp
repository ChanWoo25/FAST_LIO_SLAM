...
{
  if (voxel_size < 0.01) {
    std::cerr << "[downsampleCloud] Too small voxel size" << std::endl;
    return;
  }
  std::unordered_map<VOXEL_LOC, M_POINT> voxel_map;
  uint plsize = cloud.size();

  for (uint i = 0; i < plsize; i++) {
    pcl::PointXYZI &p_c = cloud[i];
    float loc_xyz[3];
    for (int j = 0; j < 3; j++) {
  ...
  for (auto iter = voxel_map.begin(); iter != voxel_map.end(); ++iter) {
    cloud[i].x = iter->second.xyz[0] / iter->second.count;
    cloud[i].y = iter->second.xyz[1] / iter->second.count;
    cloud[i].z = iter->second.xyz[2] / iter->second.count;
    cloud[i].intensity = iter->second.intensity / iter->second.count;
    i++;
  }
}
...
void loopFindNearKeyframesCloud( pcl::PointCloud<PointType>::Ptr& nearKeyframes, const int& key, const int& submap_size, const int& root_idx)
{
    ...
    std::cout << "ICP Filter: " << nearKeyframes->size() << std::endl;
    // downSizeFilterICP.setInputCloud(nearKeyframes);
    // downSizeFilterICP.filter(*cloud_temp);
    // *nearKeyframes = *cloud_temp;
    downsampleCloud((*nearKeyframes), 0.4);
...
void process_pg()
{
    while(1)
    {
		while ( !odometryBuf.empty() && !fullResBuf.empty() )
        {
            ...
            std::cout << "SC Filter: " << thisKeyFrame->size() << std::endl;
...
void pubMap(void)
{
    ...
    std::cout << "PGO Filter: " << laserCloudMapPGO->size() << std::endl;
    // downSizeFilterMapPGO.setInputCloud(laserCloudMapPGO);
    // downSizeFilterMapPGO.filter(*laserCloudMapPGO);
    downsampleCloud((*laserCloudMapPGO), 0.05);
