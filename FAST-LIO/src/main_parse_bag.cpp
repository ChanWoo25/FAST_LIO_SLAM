  ...
  -> rosbag::Bag
{
  ...
  try
  {
    bag.open(cfg.bag_path, rosbag::bagmode::Read);
  }
...
{
  ros::init(argc, argv, "parse_bag");
  ros::NodeHandle nh;

  nh.param<std::string>("bag_path", cfg.bag_path, "");
  ROS_ASSERT(!cfg.bag_path.empty());

  rosbag::Bag bag = openBag(cfg.bag_path);
  ...
  R_velo_wrt_imu(0,0) =  0.999998; R_velo_wrt_imu(0,1) = -0.000785; R_velo_wrt_imu(0,2) = 0.002024;
  R_velo_wrt_imu(1,0) =  0.000755; R_velo_wrt_imu(1,1) =  0.999890; R_velo_wrt_imu(1,2) = 0.014825;
  R_velo_wrt_imu(2,0) = -0.002036; R_velo_wrt_imu(2,1) = -0.014823; R_velo_wrt_imu(2,2) = 0.999888;
  t_velo_wrt_imu(0) =  0.806805;
  t_velo_wrt_imu(1) = -0.332010;
  t_velo_wrt_imu(2) =  0.796533;

  R_velo_wrt_cam(0,0) = 0.007534; R_velo_wrt_cam(0,1) = -0.999971; R_velo_wrt_cam(0,2) = -0.000617;
  R_velo_wrt_cam(1,0) = 0.014802; R_velo_wrt_cam(1,1) =  0.000728; R_velo_wrt_cam(1,2) = -0.999890;
  R_velo_wrt_cam(2,0) = 0.999862; R_velo_wrt_cam(2,1) =  0.007524; R_velo_wrt_cam(2,2) =  0.014808;
  t_velo_wrt_cam(0) = -0.004070;
  t_velo_wrt_cam(1) = -0.076316;
  t_velo_wrt_cam(2) = -0.271781;

  R0(0,0) =  0.999924; R0(0,1) = 0.009838; R0(0,2) = -0.007445;
  R0(1,0) = -0.009870; R0(1,1) = 0.999942; R0(1,2) = -0.004278;
  R0(2,0) =  0.007403; R0(2,1) = 0.004352; R0(2,2) =  0.999963;

  std::vector<std::string> SEQS = {"00"}; // {"00", "02", "05", "08"};
  for (const auto & seq: SEQS)
  {
    std::stringstream ss_lidar_dir;
    ss_lidar_dir << cfg.root_dir << "/" << seq;
    generateSubmap(ss_lidar_dir.str(), cfg.voxel_size, 4096UL);
  }
